...
namee = await vanz.getName(anu.participans[0])
bufpp = await dl.telegra(ppimg)
const canvas = require('discord-canvas')
const welcomer = await new canvas.Welcome()
.setUsername()
.setDiscriminator(namee)
...
anu = Buffer.from(base64, 'base64')
await vanz.sendFile(from, anu, 'welcome.jpg', `Welcome @${num.split('@')[0]}`, {contextInfo: {mentionedJid: [num]}})
...
bufpp = await dl.telegra(ppimg)
const canvas = require('discord-canvas')
const godbye = await new canvas.Goodbye()
.setUsername()
.setDiscriminator(namee)
.setMemberCount(mdata.participants.length)
.setGuildName(mdata.subject)
...
const base64 = `${welcomer.toBuffer().toString('base64')}`
anu = Buffer.from(base64, 'base64')
await vanz.sendFile(from, anu, 'leave.jpg', `Goodbye @${num.split('@')[0]}`, {contextInfo: {mentionedJid: [num]}})
}
} catch (e) {
...
gel3 = q.split("|")[2];
gel4 = q.split("|")[4];
await vanz.sendButtonMessage(from, gel1, gel2, gel3, gel4)
break
default:
}
} catch (e) {
